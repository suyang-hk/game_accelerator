...
- **iptables TPROXY 透明接管(接近真实,别人游戏直接可用)**
  ...
  - 拿到"原始目标 -> route 解析"后送隧道;用 ip rule / socket mark 区分
    "要加速 / 不加速"的流量。
- **TUN 虚拟网卡(最贴商业形态)**
  - Linux tun / Windows wintun / Android VPNService(手游端加速器就是它)。
  - 应用流量进 TUN,用户态程序封装进隧道:UDP 直接封装即可;若要加速 TCP,
    需用户态 TCP 栈(复杂度高,慎入)。代价是要 root / 管理员权限。
- **游戏识别 + 服务器 IP 库(让"一键加速"可用)**
  - 目标是"识别游戏 -> 自动把它所有服务器网段拉进接管范围"。
  - kv_store 正好当这个库:游戏名 / 域名 / 网段 -> route。数据靠抓包、官方列表、
    社区维护,属长期运营项。

推荐顺序:**本地收口代理 -> TPROXY -> TUN**。

### 2. 节点与多跳 —— 从"单机单点"到"多点集群"

> 现状:一个 gateway 同时当接入点+出口点,route 写死一个地址;没有候选节点集,
> 没有健康/冗余,"查上游高速节点"无从谈起。
