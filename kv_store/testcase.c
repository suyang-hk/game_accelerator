...
void array_testcase(int connfd) {
	...
	test_case(connfd, "SET Name SUYANG", "SUCCESS", "SETCase");
	test_case(connfd, "GET Name", "SUYANG", "GETCase");
	test_case(connfd, "MOD Name FISH", "SUCCESS", "MODCase");
	test_case(connfd, "GET Name", "FISH", "GETCase");
...
void rbtree_testcase_5w_node(int connfd) {
	...
	for (i = 0;i < count;i ++) {
		...
		snprintf(cmd, 128, "SET Name%d SUYANG%d", i, i);
	...
	for (i = 0;i < count;i ++) {
		...
		snprintf(cmd, 128, "DEL Name%d SUYANG%d", i, i);
